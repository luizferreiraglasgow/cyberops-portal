...
const teamConfig = {
  ...
  ddos: { label: 'DDoS/DoS', bg: '#3d1a00', border: '#f97316', color: '#fdba74' },
...
export default function ToolsPage() {
  ...
  const [filterTeam, setFilterTeam] = useState<'all' | 'red' | 'blue' | 'bug-hunting' | 'osint' | 'utility' | 'ddos'>('all')
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: 6 }}>
            {(['all', 'red', 'blue', 'bug-hunting', 'osint', 'ddos', 'utility'] as const).map(team => (
              <button key={team} onClick={() => setFilterTeam(team)}
                ...
                {team === 'all' ? 'All' : team === 'red' ? '🔴 Red Team' : team === 'blue' ? '🔵 Blue Team' : team === 'bug-hunting' ? '🟡 Bug Hunting' : team === 'osint' ? '🟢 OSINT' : team === 'ddos' ? '⚡ DDoS/DoS' : '🔵 Utility'}
              </button>
            ))}
          </div>
